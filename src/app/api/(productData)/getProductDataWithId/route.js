...
async function getPosts({ productID, reqApiKey, gXkey }) {
  try {
    ...
    const data = await productDataModel.find({ _id: productID })
      .sort({ date: 'desc' })
      .lean();
...
export const GET = async (req, res) => {
  ...
  if (!productID) {
    return NextResponse.json({
      success: false,
      message: "ProductID must be provided in the URL."
    }, { status: 400 });
  }

  try {
    const data = await getPosts({ productID, reqApiKey, gXkey });
